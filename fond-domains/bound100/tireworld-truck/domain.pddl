(define (domain tire-truck)
  (:requirements :typing :strips :non-deterministic :negative-preconditions :fluents :action-costs)
  (:types location tire)
  ...
  (:action move-car-spiky
    ...
    :effect (and (increase (total-cost) 1) (car-at ?to) (free ?from) (not (free ?to)) (not (car-at ?from)) (oneof (and) (and (not (not-flattire)))))
  )

  (:action move-car-spiky-to-initial
    :parameters (?from - location ?to - location)
    :precondition (and (initial-location ?to) (car-at ?from) (car-spiky-road ?from ?to) (not-flattire))
    :effect (and (increase (total-cost) 1) (car-at ?to) (free ?from) (free ?to) (not (car-at ?from)) (oneof (and) (and (not (not-flattire)))))
  )
  ...
  (:action move-car-normal
    ...
    :effect (and (increase (total-cost) 1) (car-at ?to) (free ?from) (not (free ?to)) (not (car-at ?from)))
  )

  (:action move-car-normal-to-initial
    :parameters (?from - location ?to - location)
    :precondition (and (initial-location ?to) (car-at ?from) (car-road ?from ?to) (not-flattire))
    :effect (and (increase (total-cost) 1) (car-at ?to) (free ?from) (free ?to) (not (car-at ?from)))
  )
  ...
  (:action move-truck
    ...
    :effect (and (increase (total-cost) 1) (truck-at ?to) (free ?from) (not (free ?to)) (not (truck-at ?from)))
  )

  (:action move-truck-to-initial
    :parameters (?from - location ?to - location)
    :precondition (and (initial-location ?to) (truck-at ?from) (truck-road ?from ?to))
    :effect (and (increase (total-cost) 1) (truck-at ?to) (free ?from) (free ?to) (not (truck-at ?from)))
  )
  ...
  (:action loadtire
    ...
    :effect (and (increase (total-cost) 1) (loaded ?t) (not (tire-at ?t ?loc)))
  )

  (:action droptire
    :parameters (?loc - location ?t - tire)
    :precondition (and (truck-at ?loc) (loaded ?t))
    :effect (and (increase (total-cost) 1) (not (loaded ?t)) (tire-at ?t ?loc))
  )

  (:action fix
    :parameters (?loc - location ?t - tire)
    :precondition (and (car-at ?loc) (not (not-flattire)) (tire-at ?t ?loc))
    :effect (and (increase (total-cost) 1) (not-flattire) (not (tire-at ?t ?loc)))
  )
